...
\[\norm{\mat{A}\vec{x}}=\sum_{i=1}^n\bigabs{\sum_{j=1}^n A_{ij}x_j}\le\sum_{i,j}\abs{A_{ij}}\]
...
\item The derivative of a matrix-valued function of scalars \(\mat{F}:\R\to\R^{n\times n}\) at \(x\in\R\) is naturally identified with the \(n\times n\) (tangent) matrix
\[\mat{F'}(x)=[F_{ij}'(x)]\]
