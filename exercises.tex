...
To ensure that \(f(s)=a\), we must have \(h(s)=[g(s)-g(t)]a\), which suggests that \(h(x)\)~should contain the term \([g(x)-g(t)]a\). Similarly to ensure that \(f(t)=b\), we must have \(h(t)=[g(s)-g(t)]b\), which suggests that \(h(x)\)~should contain the term \([g(s)-g(x)]b\). Adding these terms together, we get
\[f(x)=\frac{[g(x)-g(t)]a-[g(x)-g(s)]b}{g(s)-g(t)}\]
